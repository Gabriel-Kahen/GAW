...
Assets may be organized into single-level folders containing audio or MIDI event sources. Folder
membership is project metadata only: moving or deleting a folder never changes or deletes its
sources. Tracks may likewise be organized into named, collapsible groups within a composition.
Groups do not add buses or change render order; collapsing one only hides its member rows in the
Tracks and Timeline columns.
...
Right-clicking a materialized audio asset also exposes `CONVERT TO MIDI`. Basic Pitch analysis runs on a dedicated bounded background worker, never on the UI, persistence, or real-time audio threads. Its second-based note events are converted to project beats using the asset BPM when known and the current project BPM otherwise. Completion creates one undoable canonical `EventData` value named from the source stem plus ` (MIDI)`; repeated conversions receive a numeric suffix. MIDI event assets appear alongside audio assets with note count and musical duration, can be dragged into the arrangement as event clips, and remain exportable through the standard MIDI CLI, while temporary Basic Pitch MIDI/CSV output is not canonical project data. The source audio remains unchanged, duplicate requests for the same source are suppressed while work is pending, and results for a removed or changed source revision, name, or tempo basis are discarded. Worker shutdown cancels and reaps active inference. Basic Pitch's per-note pitch-bend arrays are intentionally omitted because canonical `EventData` currently has a single track-wide bend lane and cannot faithfully represent overlapping polyphonic bends.

Materialized audio assets also expose an `X-LANCE STEM SPLITTER` action. The user selects any of X-LANCE's eight fixed targets and may enable its denoise and vocal-dereverb restoration stages. The bundled integration targets Linux, prefers NVIDIA CUDA, selects a separate gfx1010-specific AMD ROCm runtime for cards such as the Radeon RX 5700 XT, and otherwise falls back to slower CPU inference while preserving the same model weights. On first use, GAW creates a backend-specific, versioned, hash-locked Python 3.12 runtime in app-owned data storage and reuses it across projects; executable runtime files are never loaded from a song project. Installation verifies the selected GPU with real compute kernels before publishing the runtime. Installation and inference run on a dedicated cancellable worker, report installation or per-stem progress, and use project-local temporary storage only for the current job; only a complete successful result crosses into project storage. The pinned app-managed X-LANCE checkout is cleaned and locked before use, and source/output audio metadata is validated before import. GAW then imports every selected stem in one atomic transaction, creates a logical asset folder named `SPLIT - <original filename>`, and names each child `<INSTRUMENT> - <original filename>`. Generated audio remains hash-addressed under `assets/media/`; sidebar folders are organization metadata, not mutable media directories. Identical or silent stems retain distinct logical asset IDs while safely sharing the same immutable media blob. X-LANCE's eight target estimates are not treated as a reconstructive sum.
