...
impl TempoStretcher for CanonicalTempoStretcher {
    fn stretch(
        ...
        if !input.len().is_multiple_of(channels) {
            return Err("stretch input must contain complete audio frames".into());
        }
    ...
    fn stretch_source(
        ...
        if source.channel_layout() != layout {
            return Err("stretch source layout does not match the requested layout".into());
        }
        let channels = layout.channels();
        let input_frames =
...
/// An immutable render plan paired with compiled canonical DSP definitions and sources.
#[derive(Debug)]
...
impl<'a> ProjectCompiler<'a> {
    ...
    pub fn compile(
        ...
        for composition in project.compositions.iter().filter(|composition| {
            ...
            let layout = composition.output_layout;
...
mod tests {
    ...
    use gaw_dsp::AudioLayout as DspLayout;

    #[test]
    fn canonical_stretch_rejects_partial_frames_and_mismatched_source_layouts() {
        let stretcher = CanonicalTempoStretcher;
        for samples in [3, 48_001] {
            assert!(
                stretcher
                    .stretch(&vec![0.0; samples], ChannelLayout::Stereo, 48_000, 128)
                    .is_err()
            );
        }
        let source = MemoryFrameSource::new(ChannelLayout::Mono, vec![0.0; 128]).unwrap();
        let mut emitted = false;
        assert!(
            stretcher
                .stretch_source(&source, ChannelLayout::Stereo, 48_000, 128, &mut |_| {
                    emitted = true;
                    Ok(())
                })
                .is_err()
